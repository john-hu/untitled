...
class RecipeResultSpider(BaseResultSpider):
    ...
    def parse_response(self, response: Response) -> RecipeItem:
        if not response.css('[itemprop=recipeIngredient]') or not response.css(
                '[itemprop=recipeInstructions]'):
            raise InvalidResponseData()
        item = RecipeItem(
            authors=[
                get_attribute(
                    response.css('.recipe-page meta[itemprop=author]'),
                    'content',
                    'Food Network')],
            dateCreated=get_attribute(
                response.css('meta[itemprop=datePublished]'), 'content'),
            description=get_attribute(
                response.css('meta[name=description]'), 'content'),
            id=response.request.url,
            ingredientsRaw=response.css(
                '[itemprop=recipeIngredient]::text').getall(),
            instructionsRaw=response.css(
                '[itemprop=recipeInstructions] p::text').getall(),
            language=response.css(
                'meta[property="og:locale"]').attrib['content'].strip(),
        ...
        if response.css('meta[itemprop=image]'):
            item.images = [
                response.css('meta[itemprop=image]').attrib['content'].strip()]
        if response.css('meta[itemprop=keywords]'):
            item.keywords = response.css(
                'meta[itemprop=keywords]').attrib['content'].strip().split(', ')
        ...
        if not item.instructionsRaw:
            ...
            item.instructionsRaw = response.css(
                '[itemprop=recipeInstructions] li::text').getall()
        if not item.instructionsRaw:
            # the third form of instructions @@.
            item.instructionsRaw = response.css(
                '[itemprop=recipeInstructions]::text').getall()
        if response.css('meta[itemprop=cookTime]'):
            item.cookTime = parse_duration(
                response.css('meta[itemprop=cookTime]').attrib['content'].strip())
        if response.css('meta[itemprop=prepTime]'):
            item.prepTime = parse_duration(
                response.css('meta[itemprop=prepTime]').attrib['content'].strip())
        if response.css('meta[itemprop=recipeYield]'):
            item.yield_data = parse_yield(
                response.css('meta[itemprop=recipeYield]').attrib['content'].strip())
