...
class Uploader:
    ...
    def pull_and_merge(self, recipe):
        ...
        if old_data.status_code == 200 and len(old_data.json()['docs']) > 0:
            ...
            if final_data.get('ingredients', []) and final_data.get(
                    'instructions', []):
        ...
        result = requests.post(
            self.__endpoint, data=json.dumps(
                [final_data]), auth=self.__auth)
        print(
            f'result: {result.status_code}, {result.text} for {recipe["id"]}')
